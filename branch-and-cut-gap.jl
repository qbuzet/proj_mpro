...
function main(liste)
        ...
        for file_name in liste
            if occursin("euclidean", file_name)
                ...
                obj = cutting_planes(file_name)
                exec_time = time() - start
                global_exec_time = time() - global_start
                push!(results, string(file_name, " ", obj, " ", exec_time, " ", global_exec_time))
                open("output_cutting_planes_limit.txt", "w") do f
                    write(f, string(results))
                end        
